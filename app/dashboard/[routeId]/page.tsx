...
const generateMockTimeData = (days = 30) => {
    ...
    for (let i = days; i > 0; i--) {
        ...
        data.push({
            ...
            successRate: Math.min(100, Math.max(70, 100 - Math.floor(Math.random() * 30))),
        });
    }
    return data;
};
...
const prepareResponseTimeData = (logs: LogEntry[] | null | undefined): ResponseTimeDataPoint[] => {
...
function RouteDetailsContent() {
    ...
    // Fetch route details
    const { data: routeDetails, isLoading, isError, refetch } = useQuery({
    ...
    const responseTimeData = prepareResponseTimeData(routeDetails?.logs || sampleLogsData);
        ...
        return (
            <div className="flex justify-center items-center min-h-screen">
                <div className="flex flex-col items-center space-y-4">
                    ...
                    <p className="text-lg font-semibold">Loading route data...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex justify-center items-center min-h-screen">
                <Card className="p-6 max-w-lg">
                    <h2 className="text-xl font-bold text-destructive mb-4">Error Loading Route</h2>
                    <p className="mb-4">We couldn't load the details for this route. It may have been deleted or you may not have permission to view it.</p>
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-bold">{routeDetails.name || 'Route Details'}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <Card className="p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${routeDetails.isActive ? 'bg-green-500' : 'bg-red-500'}`}></div>
                        <span>{routeDetails.isActive ? 'Active' : 'Inactive'}</span>
                    </div>
                </Card>
                <Card className="p-6 hover:shadow-md transition-shadow">
                ...
                <Card className="p-6 hover:shadow-md transition-shadow">
                    ...
                    <div>
                        {formatDate(routeDetails.createdAt)}
                    </div>
                </Card>
            </div>
            ...
            <Tabs value={tab} onValueChange={setTab} className="mb-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <Input
                                        ...
                                        defaultValue={routeDetails.method || 'GET'}
                                    />
                                </div>
                                <div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <Chip
                                            color={routeDetails.expectedStatusCode === 200 ? "success" :
                                                routeDetails.expectedStatusCode >= 400 ? "danger" :
                                                    routeDetails.expectedStatusCode >= 300 ? "warning" : "primary"}
                                        >
                                            {routeDetails.expectedStatusCode || 200}
                                        </Chip>
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    <Input
                                        ...
                                        defaultValue={routeDetails.responseTimeThreshold || 500}
                                    />
                                </div>
                                <div>
                                    <Input
                                        ...
                                        defaultValue={routeDetails.monitoringInterval || 300}
                                    />
                                </div>
                            </div>
                        </Card>
                            ...
                            <div className="h-64">
                                <ChartContainer className="h-[200px] w-full" config={{
                                    ...
                                    >
                                        ...
                                        />
                                        <YAxis dataKey="responseTime" tick={{ fontSize: 12 }} />
                                        <ChartTooltip
                                            content={
                                                <ChartTooltipContent formatter={(value) => [
                ...
                <TabsContent value="analytics">
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        {(() => {
                            ...
                            const aggregateLogData = (logs: LogEntry[] | null | undefined): AggregatedLogData[] => {
                                if (!logs || !Array.isArray(logs) || logs.length === 0) {
                                    console.warn("No log data found to aggregate.");
                                    return [];
                                }

                                const aggregated: { [date: string]: { requests: number; successfulRequests: number } } = {};

                                logs.forEach(log => {
                                    try {
                                        const date = new Date(log.createdAt).toISOString().split('T')[0];
                                        if (!aggregated[date]) {
                                            aggregated[date] = { requests: 0, successfulRequests: 0 };
                                        }
                                        aggregated[date].requests++;
                                        if (log.isSuccess === true) {
                                            aggregated[date].successfulRequests++;
                                        }
                                    } catch (e) {
                                ...
                                const result = Object.entries(aggregated)
                                    .map(([date, data]) => ({
                                        ...
                                        successRate: data.requests > 0 ? Math.round((data.successfulRequests / data.requests) * 100) : 0,
                                    }))
                            ...
                            const aggregatedData = aggregateLogData(routeDetails?.logs);
                            // Show charts only if there is data from more than one day.
                            const showCharts = aggregatedData && aggregatedData.length > 1;
                            const numberOfDays = aggregatedData.length > 0 ? aggregatedData.length : 0;
                            ...
                            return (
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <Card className="p-6 hover:shadow-md transition-shadow">
                                        <CardHeader>
                                            <CardTitle>Request Volume (Last {numberOfDays} Day{numberOfDays !== 1 ? 's' : ''})</CardTitle>
                                        </CardHeader>
                                        <div className="h-64">
                                            <ChartContainer className="h-[200px] w-full" config={{
                                                date: { label: 'Date' },
                                    ...
                                    <Card className="p-6 hover:shadow-md transition-shadow">
                                        <CardHeader>
                                            <CardTitle>Success Rate (Last {numberOfDays} Day{numberOfDays !== 1 ? 's' : ''})</CardTitle>
                                        </CardHeader>
                                        <div className="h-64">
                                            <ChartContainer className="h-[200px] w-full" config={{
                                                date: { label: 'Date' },
                                                ...
                                                >
                                                    ...
                                                    <ChartTooltip
                                                        cursor={false}
                                                        content={<ChartTooltipContent formatter={(value, name) => [`${value}%`, name]} indicator="line" />}
                                                    />
                                                    <Line
                                                        dataKey="successRate"
                                                        type="natural"
                                        ...
                                        {routeDetails.logs.map((log, i) => {
                                            ...
                                            return (
                                                <TableRow key={i}>
                                                    ...
                                                    <TableCell>
                                                        <span className={`px-2 py-1 rounded-full text-xs ${i % 5 === 0 ? 'bg-destructive/20 text-destructive' : 'bg-success/20 text-success'}`}>
                                                            {i % 5 === 0 ? '500' : '200'}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell>{Math.floor(Math.random() * 500) + 50}ms</TableCell>
                                                    <TableCell>
                                                        <div className="flex space-x-2">
                                                            ...
                                                            {log.responseTime > (routeDetails.responseTimeThreshold * 0.8) && (
                                                                <Chip color="warning">Slow</Chip>
                                                            )}
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
                ...
                <TabsContent value="settings">
                    <Card className="p-6 hover:shadow-md transition-shadow">
                        <h3 className="text-xl font-semibold mb-4">Route Settings</h3>
                        <p className="text-muted-foreground mb-6">Edit your route settings here. This is a placeholder for the route settings form.</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center space-x-3">
                                <Checkbox defaultChecked={routeDetails.isActive}>
                                    Enabled
                                </Checkbox>
                            </div>
