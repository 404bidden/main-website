...
export const POST = async (
    ...
    if (route.requestHeaders) {
        ...
        const forbiddenHeaders = [
            ...
            'x-forwarded-ssl', 'x-correlation-id', 'fastly-client-ip', 'true-client-ip',

            // Security headers that should not be forwarded
            'sec-', 'proxy-', 'cf-', 'x-csrf', 'x-xsrf', 'strict-transport-security',
            'content-security-policy', 'x-content-security-policy', 'x-webkit-csp',

            // Internal/sensitive headers
            'x-api-key', 'x-internal', 'x-secret', 'x-amz-security-token', 'api-key',
            'x-functions-key', 'x-goog-authenticated-user-email', 'x-aws-', 'x-amz-',
            'x-azure-', 'x-gcp-', 'x-heroku-', 'x-vercel-',
    ...
    if (
        ...
        /^172\.(1[6-9]|2[0-9]{1,3})\.([0-9]{1,3})\.([0-9]{1,3})$/.test(targetUrl.hostname) ||
        ...
        /^100\.(6[4-9]|[7-9][0-9]|1[0-1][0-9]|12[0-7])\.([0-9]{1,3})\.([0-9]{1,3})$/.test(targetUrl.hostname) ||
        ...
        [21, 22, 23, 25, 80, 111, 135, 137, 139, 389, 445, 1433, 1521, 3306, 3389, 5432, 5900, 6379, 9200, 11211, 27017].includes(Number(targetUrl.port)) ||
    ...
    if (route.requestBody) {
        ...
        if (bodySize > 1024 * 1024) { // 1MB limit example
